...
#define LVGL_TASK_STACK_SIZE   (8 * 1024)  // Increased stack size
#define LVGL_TASK_PRIORITY     4           // Increased priority
...
static void lvgl_task_handler(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (lvgl_lock(100)) {  // Timeout after 100ms instead of blocking indefinitely
...
lv_disp_t* lcd_ili9341_init(void)
{
    ...
    esp_err_t mirror_result = esp_lcd_panel_mirror(panel_handle, true, false);
    if (mirror_result != ESP_OK) {
        return NULL;
    }
    ESP_ERROR_CHECK(mirror_result);
