...
static const char *TAG = "TEMP_SCREEN";
...
static void temp_back_button_event_cb(lv_event_t *e);
static void temp_settings_button_event_cb(lv_event_t *e);
static void temp_calibration_button_event_cb(lv_event_t *e);
static void temp_alarm_button_event_cb(lv_event_t *e);

void temp_detail_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing Temperature detail screen");
    
    sensor_screen_t *screen = &temp_detail_screen;
    ...
    lv_obj_add_event_cb(screen->back_button, temp_back_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_label_set_text(current_label, "Current Temp:");
    ...
    lv_label_set_text(screen->current_value_label, "22.5°C");
    ...
    lv_label_set_text(target_label, "Target Temp:");
    ...
    lv_label_set_text(screen->target_value_label, "24.0°C");
    ...
    lv_chart_set_range(screen->chart, LV_CHART_AXIS_PRIMARY_Y, 15, 35);
    ...
    lv_obj_add_event_cb(screen->settings_button, temp_settings_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    ESP_LOGI(TAG, "Temperature detail screen initialized");
}

void temp_settings_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing Temperature settings screen");
    
    sensor_screen_t *screen = &temp_settings_screen;
    if (screen->is_initialized) return;
    ...
    lv_obj_set_style_text_color(title_label, lv_color_hex(0xffffff), 0);
    lv_obj_set_style_text_font(title_label, &lv_font_montserrat_14, 0);
    lv_label_set_text(title_label, "Temperature Settings");
    lv_obj_align(title_label, LV_ALIGN_CENTER, 0, 0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        switch (i) {
            case 0: // Calibration
                lv_obj_add_event_cb(item, temp_calibration_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            case 1: // Alarm Thresholds
                lv_obj_add_event_cb(item, temp_alarm_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            default:
                break;
        }
    ...
    screen->is_initialized = true;
    ESP_LOGI(TAG, "Temperature settings screen initialized");
}
...
static void temp_back_button_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Temperature back button clicked");
...
static void temp_settings_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Temperature settings button clicked");
        // Переход к экрану настроек температуры
        if (!temp_settings_screen.is_initialized) {
            temp_settings_screen_init();
        }
        lv_screen_load(temp_settings_screen.screen);
    }
}
...
static void temp_calibration_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Temperature calibration button clicked");
        // Здесь будет логика калибровки температуры
    }
}
...
static void temp_alarm_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Temperature alarm button clicked");
        // Здесь будет логика настройки аварийных порогов температуры
    }
}

// Функция обновления данных температуры
void temp_update_data(float current_value, float target_value)
{
    temp_data.current_value = current_value;
    temp_data.target_value = target_value;
    
    if (temp_detail_screen.is_initialized) {
        char current_text[32];
        char target_text[32];
        
        snprintf(current_text, sizeof(current_text), "%.1f°C", current_value);
        snprintf(target_text, sizeof(target_text), "%.1f°C", target_value);
        
        lv_label_set_text(temp_detail_screen.current_value_label, current_text);
        lv_label_set_text(temp_detail_screen.target_value_label, target_text);
    }
}
