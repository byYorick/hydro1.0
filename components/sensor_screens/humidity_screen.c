...
static const char *TAG = "HUMIDITY_SCREEN";

// Данные для влажности
typedef struct {
    float current_value;
    float target_value;
    float min_value;
...
static humidity_data_t humidity_data = {
    .current_value = 65.0f,
    .target_value = 70.0f,
    .min_value = 30.0f,
    .max_value = 90.0f,
...
static void humidity_back_button_event_cb(lv_event_t *e);
static void humidity_settings_button_event_cb(lv_event_t *e);
static void humidity_calibration_button_event_cb(lv_event_t *e);
static void humidity_alarm_button_event_cb(lv_event_t *e);

void humidity_detail_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing Humidity detail screen");
    
    sensor_screen_t *screen = &humidity_detail_screen;
    ...
    lv_obj_add_event_cb(screen->back_button, humidity_back_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_label_set_text(current_label, "Current Humidity:");
    ...
    lv_label_set_text(screen->current_value_label, "65.0%");
    ...
    lv_label_set_text(target_label, "Target Humidity:");
    ...
    lv_label_set_text(screen->target_value_label, "70.0%");
    ...
    lv_chart_set_range(screen->chart, LV_CHART_AXIS_PRIMARY_Y, 30, 90);
    ...
    lv_obj_add_event_cb(screen->settings_button, humidity_settings_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    ESP_LOGI(TAG, "Humidity detail screen initialized");
}

void humidity_settings_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing Humidity settings screen");
    
    sensor_screen_t *screen = &humidity_settings_screen;
    if (screen->is_initialized) return;
    ...
    lv_obj_set_style_text_color(title_label, lv_color_hex(0xffffff), 0);
    lv_obj_set_style_text_font(title_label, &lv_font_montserrat_14, 0);
    lv_label_set_text(title_label, "Humidity Settings");
    lv_obj_align(title_label, LV_ALIGN_CENTER, 0, 0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        switch (i) {
            case 0: // Calibration
                lv_obj_add_event_cb(item, humidity_calibration_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            case 1: // Alarm Thresholds
                lv_obj_add_event_cb(item, humidity_alarm_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            default:
                break;
        }
    ...
    screen->is_initialized = true;
    ESP_LOGI(TAG, "Humidity settings screen initialized");
}
...
static void humidity_back_button_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Humidity back button clicked");
...
static void humidity_settings_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Humidity settings button clicked");
        // Переход к экрану настроек влажности
        if (!humidity_settings_screen.is_initialized) {
            humidity_settings_screen_init();
        }
        lv_screen_load(humidity_settings_screen.screen);
    }
}
...
static void humidity_calibration_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Humidity calibration button clicked");
        // Здесь будет логика калибровки влажности
    }
}
...
static void humidity_alarm_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "Humidity alarm button clicked");
        // Здесь будет логика настройки аварийных порогов влажности
    }
}

// Функция обновления данных влажности
void humidity_update_data(float current_value, float target_value)
{
    humidity_data.current_value = current_value;
    humidity_data.target_value = target_value;
    
    if (humidity_detail_screen.is_initialized) {
        char current_text[32];
        char target_text[32];
        
        snprintf(current_text, sizeof(current_text), "%.1f%%", current_value);
        snprintf(target_text, sizeof(target_text), "%.1f%%", target_value);
        
        lv_label_set_text(humidity_detail_screen.current_value_label, current_text);
        lv_label_set_text(humidity_detail_screen.target_value_label, target_text);
    }
}
