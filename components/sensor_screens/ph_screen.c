...
static void ph_back_button_event_cb(lv_event_t *e);
static void ph_settings_button_event_cb(lv_event_t *e);
static void ph_calibration_button_event_cb(lv_event_t *e);
static void ph_alarm_button_event_cb(lv_event_t *e);

void ph_detail_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing pH detail screen");
    
    // Инициализируем базовый экран
    sensor_screen_t *screen = &ph_detail_screen;
    ...
    lv_obj_add_event_cb(screen->back_button, ph_back_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_label_set_text(current_label, "Current pH:");
    ...
    lv_label_set_text(target_label, "Target pH:");
    ...
    lv_chart_set_range(screen->chart, LV_CHART_AXIS_PRIMARY_Y, 4, 9);
    ...
    lv_obj_add_event_cb(screen->settings_button, ph_settings_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    ESP_LOGI(TAG, "pH detail screen initialized");
}

void ph_settings_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing pH settings screen");
    
    sensor_screen_t *screen = &ph_settings_screen;
    if (screen->is_initialized) return;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        switch (i) {
            case 0: // Calibration
                lv_obj_add_event_cb(item, ph_calibration_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            case 1: // Alarm Thresholds
                lv_obj_add_event_cb(item, ph_alarm_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            default:
                break;
        }
    ...
    screen->is_initialized = true;
    ESP_LOGI(TAG, "pH settings screen initialized");
}
...
static void ph_back_button_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "pH back button clicked");
...
static void ph_settings_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "pH settings button clicked");
        // Переход к экрану настроек pH
...
static void ph_calibration_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "pH calibration button clicked");
        // Здесь будет логика калибровки pH
    }
}
...
static void ph_alarm_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "pH alarm button clicked");
        // Здесь будет логика настройки аварийных порогов pH
    }
}

// Функция обновления данных pH
void ph_update_data(float current_value, float target_value)
{
    ph_data.current_value = current_value;
    ph_data.target_value = target_value;
    
    if (ph_detail_screen.is_initialized) {
        ...
        lv_label_set_text(ph_detail_screen.current_value_label, current_text);
        lv_label_set_text(ph_detail_screen.target_value_label, target_text);
    }
}
