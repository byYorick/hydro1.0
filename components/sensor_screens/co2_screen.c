...
static const char *TAG = "CO2_SCREEN";
...
static void co2_back_button_event_cb(lv_event_t *e);
static void co2_settings_button_event_cb(lv_event_t *e);
static void co2_calibration_button_event_cb(lv_event_t *e);
static void co2_alarm_button_event_cb(lv_event_t *e);

void co2_detail_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing CO2 detail screen");
    
    sensor_screen_t *screen = &co2_detail_screen;
    ...
    lv_obj_add_event_cb(screen->back_button, co2_back_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_label_set_text(title_label, "CO2 Level");
    ...
    lv_label_set_text(current_label, "Current CO2:");
    ...
    lv_label_set_text(screen->current_value_label, "420 ppm");
    ...
    lv_label_set_text(target_label, "Target CO2:");
    ...
    lv_label_set_text(screen->target_value_label, "450 ppm");
    ...
    lv_chart_set_range(screen->chart, LV_CHART_AXIS_PRIMARY_Y, 300, 1000);
    ...
    lv_obj_add_event_cb(screen->settings_button, co2_settings_button_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    ESP_LOGI(TAG, "CO2 detail screen initialized");
}

void co2_settings_screen_init(void)
{
    ESP_LOGI(TAG, "Initializing CO2 settings screen");
    
    sensor_screen_t *screen = &co2_settings_screen;
    if (screen->is_initialized) return;
    ...
    lv_obj_set_style_text_color(title_label, lv_color_hex(0xffffff), 0);
    lv_obj_set_style_text_font(title_label, &lv_font_montserrat_14, 0);
    lv_label_set_text(title_label, "CO2 Settings");
    lv_obj_align(title_label, LV_ALIGN_CENTER, 0, 0);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        switch (i) {
            case 0: // Calibration
                lv_obj_add_event_cb(item, co2_calibration_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            case 1: // Alarm Thresholds
                lv_obj_add_event_cb(item, co2_alarm_button_event_cb, LV_EVENT_CLICKED, NULL);
                break;
            default:
                break;
        }
    ...
    screen->is_initialized = true;
    ESP_LOGI(TAG, "CO2 settings screen initialized");
}
...
static void co2_back_button_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "CO2 back button clicked");
...
static void co2_settings_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "CO2 settings button clicked");
        // Переход к экрану настроек CO2
        if (!co2_settings_screen.is_initialized) {
            co2_settings_screen_init();
        }
        lv_screen_load(co2_settings_screen.screen);
    }
}
...
static void co2_calibration_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "CO2 calibration button clicked");
        // Здесь будет логика калибровки CO2
    }
}
...
static void co2_alarm_button_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_CLICKED) {
        ESP_LOGI(TAG, "CO2 alarm button clicked");
        // Здесь будет логика настройки аварийных порогов CO2
    }
}

// Функция обновления данных CO2
void co2_update_data(float current_value, float target_value)
{
    co2_data.current_value = current_value;
    co2_data.target_value = target_value;
    
    if (co2_detail_screen.is_initialized) {
        char current_text[32];
        char target_text[32];
        
        snprintf(current_text, sizeof(current_text), "%.0f ppm", current_value);
        snprintf(target_text, sizeof(target_text), "%.0f ppm", target_value);
        
        lv_label_set_text(co2_detail_screen.current_value_label, current_text);
        lv_label_set_text(co2_detail_screen.target_value_label, target_text);
    }
}
