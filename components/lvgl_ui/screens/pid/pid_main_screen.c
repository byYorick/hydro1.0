...
{
    (void)context;
    ESP_LOGD(TAG, "Создание главного экрана PID");
    ...
    if (!screen) {
        ESP_LOGE(TAG, "Failed to create PID main screen");
        return NULL;
    }
    ...
    if (!config) {
        ESP_LOGE(TAG, "Failed to get system config!");
        return NULL;
    }
    ...
    for (int i = 0; i < PUMP_INDEX_COUNT; i++) {
        ...
        if (!pid_item) {
            ESP_LOGE(TAG, "Failed to create pid_item for PID %d", i);
        ...
        widget_add_click_handler(pid_item, on_pid_item_click, (void*)(intptr_t)i);
        ...
        if (!top_row) {
            ESP_LOGE(TAG, "Failed to create top_row for PID %d", i);
            continue;
        }
        ...
        bool enabled = config->pump_pid[i].enabled;
    ...
    ESP_LOGD(TAG, "Главный экран PID создан");
...
esp_err_t pid_main_screen_on_show(lv_obj_t *screen, void *params)
{
    (void)screen;
    (void)params;
    ESP_LOGD(TAG, "PID main screen shown");
    return ESP_OK;
}
