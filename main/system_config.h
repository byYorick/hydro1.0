 ...
 * 
 * Каждый насос управляется через оптопару и полевой транзистор (1 GPIO пин):
 * - GPIO HIGH (3.3V) = оптопара открыта → транзистор открыт → насос включен
 * - GPIO LOW (0V) = оптопара закрыта → транзистор закрыт → насос выключен
 * 
 * Подключение: ESP32-S3 GPIO → Оптопара (PC817/4N35) → Полевой транзистор (IRLZ44N/IRF540) → Насос 12V
 ******************************************************************************/

// Насос pH UP (повышение pH)
#define PUMP_PH_UP_PIN          1           // GPIO1 - Управление насосом pH UP

// Насос pH DOWN (понижение pH)
#define PUMP_PH_DOWN_PIN        2           // GPIO2 - Управление насосом pH DOWN

// Насос EC A (раствор A)
#define PUMP_EC_A_PIN           3           // GPIO3 - Управление насосом EC A

// Насос EC B (раствор B)
#define PUMP_EC_B_PIN           7           // GPIO7 - Управление насосом EC B

// Насос EC C (раствор C)
#define PUMP_EC_C_PIN           8           // GPIO8 - Управление насосом EC C

// Насос WATER (подача чистой воды)
#define PUMP_WATER_PIN          16          // GPIO16 - Управление насосом WATER
...
#define TASK_STACK_SIZE_SENSOR      4096    // Задача чтения датчиков (16KB)
#define TASK_STACK_SIZE_DISPLAY     2816    // Задача обновления дисплея (11KB) - уменьшено для WiFi
#define TASK_STACK_SIZE_NOTIFICATION 2560   // Задача уведомлений (10KB)
#define TASK_STACK_SIZE_DATALOGGER  3584    // Задача логирования (14KB)
#define TASK_STACK_SIZE_SCHEDULER   2048    // Задача планировщика (8KB)
#define TASK_STACK_SIZE_PH_EC       2048    // Задача pH/EC контроля (8KB)
#define TASK_STACK_SIZE_ENCODER     1792    // Задача энкодера (7KB) - уменьшено для WiFi
...
#define QUEUE_SIZE_ENCODER          50      // Очередь событий энкодера (увеличена для быстрого вращения)
#define QUEUE_SIZE_NOTIFICATIONS    20      // Очередь уведомлений (увеличена)
